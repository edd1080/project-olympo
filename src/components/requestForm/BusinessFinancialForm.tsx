...
const BusinessFinancialForm: React.FC<BusinessFinancialFormProps> = ({ formData, updateFormData }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Additional Financial Information (for amounts > Q20,000) */}
      {parseFloat(formData.requestedAmount) > 20000 && (
        <>
          {/* Fixed Assets */}
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
            ...
            <CardContent>
              <div className="space-y-4">
                {(formData.fixedAssets || []).map((asset: any, index: number) => (
                  <div key={asset.id} className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 border rounded-md">
                    <div className="space-y-2">
                      <Label>Descripción *</Label>
                      <Input
                        value={asset.description || ''}
                        onChange={(e) => updateFixedAsset(asset.id, 'description', e.target.value)}
                    ...
                    <div className="space-y-2">
                      ...
                      <Select 
                        ...
                        onValueChange={(value) => updateFixedAsset(asset.id, 'condition', value)}
                      >
                        <SelectTrigger>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle>Pasivos / Deudas</CardTitle>
            ...
            <CardContent>
              <div className="space-y-4">
                {(formData.liabilities || []).map((liability: any, index: number) => (
                  <div key={liability.id} className="grid grid-cols-1 md:grid-5 gap-4 p-4 border rounded-md">
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        value={liability.purpose || ''}
                        onChange={(e) => updateLiability(liability.id, 'purpose', e.target.value)}
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-muted/20 p-4 rounded-md text-center">
                  <div className="text-2xl font-bold text-green-600">Q {calculateTotalAssets().toFixed(2)}</div>
                  <div className="text-sm text-muted-foreground">Total Activos</div>
                </div>
                <div className="bg-muted/20 p-4 rounded-md text-center">
                  <div className="text-2xl font-bold text-red-600">Q {calculateTotalLiabilities().toFixed(2)}</div>
                  <div className="text-sm text-muted-foreground">Total Pasivos</div>
                </div>
                <div className="bg-muted/20 p-4 rounded-md text-center">
                  <div className="text-2xl font-bold text-blue-600">Q {calculateEquity().toFixed(2)}</div>
                  <div className="text-sm text-muted-foreground">Patrimonio</div>
                </div>
              </div>
            </CardContent>
          </Card>
